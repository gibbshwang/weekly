...
export default function ScoreGauge() {
  ...
  const cy = 150;
  const r = 110;
  // Arc spans from 210deg to 330deg (left to right, semi-circle bottom)
  // Using 180deg arc: from -180 to 0 (top of circle, left to right)
  // We use 190deg sweep: from startAngle to endAngle
  const startAngle = -210; // degrees, starting from left
  const endAngle = 30;
        ...
        <div className="flex flex-col items-center gap-2 -mt-2">
          ...
          <div className="flex items-center gap-3 text-sm text-gray-400">
            ...
            <span className="flex items-center gap-1" style={{ color: change < 0 ? "#ef4444" : "#22c55e" }}>
              {change < 0 ? "▼" : "▲"} {Math.abs(change)}pt <span className="text-gray-500">(전일 대비)</span>
            </span>
          </div>
          <p className="text-xs text-gray-600 mt-1">
            자체 근사 지수 (CNN 7대 지표 기반)
          </p>
        </div>

        {/* VIX chip */}
        <div className="mt-3 flex items-center gap-2 bg-gray-800 rounded-full px-4 py-1.5">
          <span className="text-xs text-gray-500">VIX</span>
          <span className="text-sm font-bold text-red-400">{mockScore.vix_raw}</span>
        </div>
      </div>
    </div>
  );
