...
describe('getConsensusSummary', () => {
  it('returns fallback when cases array is empty', () => {
    ...
    expect(result.headline).toBe('유사 사례가 충분하지 않습니다');
  ...
  it('computes win rate from KOSPI 90d returns', () => {
    const cases: SimilarCaseWithReturns[] = [
    ...
    expect(result.headline).toContain('3건 중 2건');
  });
  ...
  it('win rate is 0 when all KOSPI 90d returns are negative', () => {
    ...
    expect(result.winRate90d).toBe(0);
    expect(result.headline).toContain('2건 중 0건');
  });
  ...
  it('treats null KOSPI 90d as non-win', () => {
    const cases: SimilarCaseWithReturns[] = [
      makeCase({ kospi90d: null }),
      makeCase({ kospi90d: 10.0 }),
    ];

    const result = getConsensusSummary(cases);

    expect(result.winRate90d).toBe(0.5);
  });

  it('computes average returns across cases', () => {
    const cases: SimilarCaseWithReturns[] = [
      {
        date: '2024-01-01', score: 20, note: 'test',
        returns: [
      ...
      {
        date: '2023-01-01', score: 18, note: 'test2',
        returns: [
  ...
  it('returns all 4 narrative assets in avgReturns', () => {
    const cases: SimilarCaseWithReturns[] = [makeCase({ kospi90d: 10 })];
    const result = getConsensusSummary(cases);
  ...
  it('handles single case correctly', () => {
    const single: SimilarCaseWithReturns = {
      date: '2024-08-05', score: 21, note: 'single test',
      returns: [
...
function makeCase(opts: { kospi90d: number | null }): SimilarCaseWithReturns {
  return {
    date: '2024-01-01',
    score: 20,
    note: 'test case',
